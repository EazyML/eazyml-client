...
def ez_download(auth_token, filename, options=None, local_filepath=None):
    ...
    filename : type 'str'
        Name of the file you want to download from eazyml cloud
    local_filepath : type 'str'
        [Optional] Full filepath where to download the file on local machine
    ...
    """
    API_REQUEST_URL = API_URL + "/ez_download"
    status_code = 500
    headers = {
        ...
        if os.path.exists(filename)\
           and response.status_code == 200:
            print('File downloaded successfully at:',
                  filename)
        elif os.path.exists(os.getcwd() + "\\" + filename)\
           and response.status_code == 200:
            print('File downloaded successfully at:',
                  os.getcwd() + "\\" + filename)
        else:
...
def ez_transfer(auth_token, file_path, options=None):
    """EazyML Authorization Function
    ...
    auth_token : type 'str'
        The auth token generated by ez_auth function
    file_path: type 'str'
        Full filepath of the file you want to upload from local machine
    options    : type 'dict'
        A Dictionary of format
        options = {
            "filename"    : <Filename to be saved on the EazyML server|None>
            }
    ...
    """
    API_REQUEST_URL = API_URL + "/ez_transfer"
    status_code = 500
    if os.path.isdir(file_path):
        ...
        for each_file in files_list:
            ...
            offset = 0
            headers = {}
            content_path = os.path.abspath(file_path + "/" + each_file)
            content_size = os.stat(content_path).st_size
            f = open(content_path, "rb")
            if "foldername" not in options:
                options["foldername"] = os.path.basename(file_path)
            if first_file:
                options["createfolder"] = True
            else:
                options["createfolder"] = False
            options["filename"] = each_file
            payload = {"options": json.dumps(options)}
            for chunk in read_in_chunks(f):
                offset = index + len(chunk)
                headers['Content-Range'] = '%s-%s/%s' % (index, offset - 1,
                                                         content_size)
                headers['Authorization'] = "Bearer " + str(auth_token)
                index = offset
                try:
                    file_chunk = {"filename": chunk}
                    ...
                    return exception_return(e, status_code)
            first_file = False 
            uploaded_files.append(response_json["filename"])
        response_json["filename"] = uploaded_files
        response_json["file_path"] = response_json["file_path"] + options["foldername"]
        ...
        if "filename" not in options:
            options["filename"] = os.path.basename(file_path)
