    ...
    try:
        ...
        try:
            ...
            return {'message': "The model has been exported to a file with " + \
                               "name '{0}' and is downloaded successfully".format(filename),
...
def ez_explain_pdf(auth_token,
                   mode,
                   outcome,
                   test_data_with_prediction,
                   training_samples_with_predictions,
                   test_samples_with_predictions,
                   record_numbers,
                   options=None):
    """EazyML Explanation Function
    ...
    auth_token : type 'str'
        The auth token generated by ez_auth function
    mode : type 'str'
        mode of the dataset - "classification"/"regression"
    outcome : type 'str'
        Name of the outcome column
    test_data_with_prediction : type 'str'
        The filename of the test dataset. Must be an absolute path
    training_samples_with_predictions : type 'str'
        The filename of the training samples. Must be an absolute path
    test_samples_with_predictions : type 'str'
        The file path of the test samples in same order of record_numbers
    record_numbers : type 'list'
        The list of record numbers of the prediction dataset
        for which explanation is needed
    options : type 'dict'
        Please provide scaler file path in options if used in model building
        options = {"scaler": <scaler_file_path"}
    ...
    return_response: type 'dict'
        ...
        Where:
            success         : True if call successful else False
            explanations : Returns explanation of the record number.
    ...
    status_code = 500
    try:
        API_REQUEST_URL = API_URL + "/ez_explain_pdf"
        payload = {
            "mode": mode,
            "outcome": outcome,
        ...
        files = [("test_data_with_prediction",
                  open(test_data_with_prediction, "rb")),
                 ("training_samples_with_predictions",
                  open(training_samples_with_predictions, "rb"))]
        test_samples_with_predictions = [
            x.strip() for x in test_samples_with_predictions.split(",")
        ]
        if not isinstance(record_numbers, list):
            print("Please provide record numbers list")
            return None
        if len(test_samples_with_predictions) != len(record_numbers):
            print("Please provide test samples file path with predictions",
                  "for all the record numbers in the same order")
            return None

        # check if test sample contains all the record numbers
        for test_samples, rec in zip(test_samples_with_predictions,
                                     record_numbers):
            files.append(("test_samples_for_record_" + str(rec) +
                          "_with_predictions", open(test_samples, "rb")))
        if options is not None:
            if "scaler" in options and options["scaler"] is not None:
                scaler = options["scaler"]
                files.append(("scaler", open(scaler, "rb")))
        headers = {
            "Authorization": "Bearer " + str(auth_token),
        }
